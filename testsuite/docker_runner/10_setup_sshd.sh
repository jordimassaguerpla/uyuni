#!/bin/bash
 ...
 docker exec sshminion bash -c "echo 'root:linux' | chpasswd && echo 123456789 > /etc/machine-id" && \
 docker exec sshminion bash -c "if [ ! -d /root/.ssh ];then mkdir /root/.ssh/;chmod 700 /root/.ssh;fi;cp /tmp/authorized_keys /root/.ssh/" && \
